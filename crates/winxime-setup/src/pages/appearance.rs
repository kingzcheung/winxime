...
pub fn render(settings: Entity<SettingsState>, cx: &mut Context<SettingsApp>) -> AnyElement {
    let (font_size, candidate_count, show_code_hint, horizontal, color_scheme, color_schemes, color_schemes_loaded, colors) = cx.read_entity(&settings, |state, _| {
        (
    ...
    SettingsPage::new("外观", colors.clone())
        .group(
            SettingsGroup::new("主题设置", colors.clone())
                .custom_item(
                    SettingsItem::render_custom(&colors, "配色方案".to_string(), Some("选择输入法的主题颜色".to_string()),
                        div()
                            .flex()
                            .flex_wrap()
                            .gap(px(12.0))
        ...
        .group(
            SettingsGroup::new("候选栏设置", colors.clone())
                .items(vec![
                    ...
                    SettingsItem::new("水平布局", 
                        SettingsControl::switch_with(horizontal,
                            move |val, _window, cx| {
