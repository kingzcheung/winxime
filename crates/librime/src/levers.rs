...
impl CustomSettings {
    ...
    pub fn set_double(&self, key: &str, value: f64) -> Result<()> {
        unsafe {
            let customize_double = (*self.api)
                .customize_double
                .ok_or(Error::FunctionNotAvailable("customize_double"))?;
            let key_c = CString::new(key)?;
    ...
    unsafe {
        ...
        if let Some(deploy_config) = (*api).deploy_config_file {
            ...
            let distribution_version =
                CString::new("customization/distribution_version").unwrap_or_default();
